...
class my_dash_components:
    ...
    def slider(label="", id="", min=0, max=10, step=1):
                ...
                [
                    dbc.Label(label, html_for=id, className="col-form-label col-sm"),
                     dbc.FormGroup(className = "col-sm", 
                        children = [
                            dcc.Slider(id=id, min=min, max=max, step=step, value=0, 
                                className="col-sm"),
                            html.Div(id='slider-output-container-'+id, className = "col-sm align-middle")
                        ], 
                        row = True
                    ),
        ...
        @app.callback(
            dash.dependencies.Output('slider-output-container-'+id, 'children'),
