...
mfr_lbl = run_query("select DISTINCT ON (fabricante) fabricante, nombre_fabricante from historicopedidos",
                    engine).dropna()
prod_cat_lbl = run_query("select MIN(jerarquia_productos), nombre_cat from categorias group by nombre_cat",
                         engine).dropna()
prod_sub_cat_lbl = run_query("select MIN(subcategoria), nombre_sub from categorias group by nombre_sub",
                             engine).dropna()
        ...
        dbc.Row(
            [
                dbc.Col(dcc.Dropdown(id="city", options=[{'label': i, 'value': i} for i in city_lbl],
                        placeholder="Select a SKU"), width=3, style={'font-size': "80%"}),
                dbc.Col(dcc.Dropdown(id="manufacturer", options=[
                    {'label': mfr_lbl.iloc[i]['nombre_fabricante'], 'value': mfr_lbl.iloc[i]['fabricante']} for i in
                ...
                dbc.Col(dcc.Dropdown(id="product sub-category", options=[
                    {'label': prod_sub_cat_lbl.iloc[i]['nombre_sub'], 'value': prod_sub_cat_lbl.iloc[i]['min']} for i in
