...
def update_graph(start_date, end_date, city, manufacturer, cat, sub_cat):
    query = "select SUM(cantidad_pedido) as cantidad, " \
            "c.Texto_breve_de_material1 from historicopedidos h left join " \
            "tiendas t on h.tienda = t.tienda left join categorias c on " \
            "h.Material = c.Material WHERE Valor_TotalFactura >0 AND " \
            "Fecha_Pedido >= " + "\'" + start_date + "\'" + " AND " \
                                                            "Fecha_Pedido < " \
                                                            "" + "\'" + \
            end_date + "\'" + ""
    ...
    df = run_query(
        query + "group by c.material, c.Texto_breve_de_material1 order by "
...
def update_moneygraph(start_date, end_date, city, manufacturer, cat, sub_cat):
    query = "select SUM(Valor_TotalFactura) as ventas, " \
            "c.Texto_breve_de_material1 from historicopedidos h left join " \
            "tiendas t on h.tienda = t.tienda left join categorias c on " \
            "h.Material = c.Material WHERE Valor_TotalFactura >0 AND " \
            "Fecha_Pedido >= " + "\'" + start_date + "\'" + " AND " \
    ...
    if sub_cat is not None:
        query = query + 'AND c.subcategoria =' + str(sub_cat)
    df = run_query(
        query + "group by c.material, c.Texto_breve_de_material1 order by ventas DESC limit 10",
        engine)
